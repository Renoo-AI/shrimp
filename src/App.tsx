...
export default function App() {
  return (
    <>
      <StructuredData />
      <main id="main-content" className="min-h-screen antialiased bg-white text-navy font-sans" role="main">
        <Navbar />
        ...
        <section id="hero" className="relative min-h-screen flex items-center justify-center overflow-hidden" style={{ background: '#0A1F3F' }}>
          ...
          <div className="relative z-10 flex flex-col items-center text-center px-6 max-w-3xl w-full">
            <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ duration: 1.5, ease: [0.16, 1, 0.3, 1] }}>
              <img src="/logo.png" alt="Shrimp Time logo — shrimp with lemon and عيش التجربة" className="w-[200px] md:w-[300px] h-auto mx-auto mb-12" style={{ filter: 'drop-shadow(0 4px 20px rgba(245,211,0,0.2))' }} />
            </motion.div>

            <motion.p initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 1, delay: 0.4 }}
              className="text-olive font-sans text-lg uppercase tracking-[2px] mb-10 font-semibold" aria-label="Vivez l'expérience">
              Vivez l'expérience
            </motion.p>

            <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 1, delay: 0.8 }}
              className="flex flex-col sm:flex-row gap-4">
              <button onClick={() => go('menu')} className="btn-yellow text-base px-10 py-4" aria-label="Voir le menu">
                📖 Voir le Menu
              </button>
              <button onClick={() => go('reservation')} className="btn-outline-white text-base px-10 py-4" aria-label="Réserver une table">
                📞 Réserver
              </button>
            </motion.div>
          </div>
        ...
        <footer className="py-20 px-6 md:px-10" style={{ background: '#0A1F3F' }} role="contentinfo">
          <div className="max-w-[1200px] mx-auto text-center">
            ...
            <p className="text-sm text-white/50 mb-4">📞 <a href={`tel:${BRANCHES[0].phone}`} className="hover:text-yellow transition-colors">{BRANCHES[0].phoneDisplay}</a> · ✉️ <a href={`mailto:${RESTAURANT_EMAIL}`} className="hover:text-yellow transition-colors">{RESTAURANT_EMAIL}</a></p>
            <p className="text-sm text-white/30 mb-6">{RESTAURANT_HOURS}</p>
            <div className="flex items-center justify-center gap-6 mb-8">
              <a href={INSTAGRAM_URL} target="_blank" rel="noopener noreferrer" className="text-sm text-white/50 hover:text-yellow transition-colors font-sans font-semibold" aria-label="Instagram @shrimp_.time">📸 @shrimp_.time</a>
              <a href="https://www.facebook.com/profile.php?id=61559768967974" target="_blank" rel="noopener noreferrer" className="text-sm text-white/50 hover:text-yellow transition-colors font-sans font-semibold" aria-label="Facebook Shrimp Time">🟦 Facebook</a>
            </div>
